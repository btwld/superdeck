...
| Runtime/presentation config split | `DeckOptions` mixes presentation composition and embedded watch; `DeckWorkspace` carries local paths | `DeckSource` owns content origin, `DeckRuntimeConfig` owns startup-only operational paths, and `DeckTheme` owns render composition | frozen | `packages/runtime_flutter`, `packages/migration_tools`, `packages/contracts` | `DeckOptions` survives only as migration scaffolding if needed; it is not the canonical v2 surface | API audit, runtime tests, migration tests |
