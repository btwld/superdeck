...
| Primary runtime bootstrap surface | `SuperDeckApp(options: DeckOptions, configuration: DeckWorkspace?)` plus `SuperDeckApp.initialize()` | `initializeSuperDeck()` followed by `SuperDeckProvider(config: ..., builder: (context, deck) => SuperDeckApp(deck: deck, theme: ..., extensions: ...))` | frozen | `packages/runtime_flutter`, `packages/migration_tools` | `SuperDeckApp.initialize()` and options/config bootstrap are no longer canonical; migration guidance owns the move to provider-first deck loading with explicit `deck` rendering | API audit, runtime tests, migration tests |
| Runtime/presentation config split | `DeckOptions` mixes presentation composition and embedded watch; `DeckWorkspace` carries local paths | `DeckConfig` owns deck origin, local watch policy, and startup-only paths; `DeckTheme` owns render composition; `DeckWorkspace` remains an internal storage/runtime helper | frozen | `packages/runtime_flutter`, `packages/migration_tools`, `packages/contracts` | `DeckOptions` is removed from the canonical surface; `DeckWorkspace` is not promoted as public app bootstrap API | API audit, runtime tests, migration tests |
| Embedded watch API guidance | `DeckOptions.watchForChanges` is the current runtime-facing switch for embedded source rebuilds | Embedded watch moves to `DeckConfig.local(watch: true)` and is no longer a render-option concern | frozen | `packages/runtime_flutter`, `packages/cli`, `packages/migration_tools`, `packages/contracts` | Use `DeckConfig.local(watch: ...)` as the canonical term and treat `watchForChanges` only as legacy migration language | runtime tests, migration tests, API audit |
...
| Advanced control surface | Advanced consumers historically reached into `DeckController` directly or through transitional handle APIs | `SuperDeck.of(context)` returning `DeckController` is the canonical advanced-control surface | frozen | `packages/runtime_flutter`, `packages/migration_tools` | Remove handle-layer indirection and keep advanced control on the single context accessor that matches the implementation | API audit, runtime tests |
