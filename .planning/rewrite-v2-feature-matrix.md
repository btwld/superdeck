...
| Style merge from code + YAML | `packages/superdeck/lib/src/styling/schema/style_config.dart` | `StyleConfigLoader` can load `styles.yaml` and merge it with code-defined styles, with code winning conflicts; this is not automatic runtime startup behavior today | `packages/runtime_flutter` | preserve | covered | runtime tests | external YAML loading stays opt-in and explicitly deferred from the current runtime-local contract |
...
| Plugin routes/actions/floating action | `packages/superdeck/lib/src/deck/superdeck_plugin.dart`, `packages/superdeck/lib/src/ui/app_shell.dart`, `packages/superdeck/lib/src/ui/panels/bottom_bar.dart` | Plugins contribute routes, inline actions, floating action, and async init | `packages/runtime_flutter` | preserve with stronger lifecycle typing | covered | runtime tests | canonical v2 behavioral surface is `DeckExtension` |
...
| Runtime bootstrap | `packages/superdeck/lib/src/utils/app_initialization.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart` | Syntax highlighting, desktop window init, and plugin init happen before app run | `packages/runtime_flutter` | preserve with cleanup | covered | runtime tests | canonical v2 bootstrap is `SuperDeckRuntime.create(...)` followed by `SuperDeckApp(runtime: ...)` |
...
| `build` one-shot flow | `packages/cli/lib/src/commands/build_command.dart` | Runs setup checks, build pipeline, and writes artifacts | `packages/cli`, `packages/build_engine`, `packages/runtime_flutter` | change intentionally | covered | integration tests | public v2 day-to-day build ownership moves to `SuperDeckRuntime.create(source: DeckSource.local(...))`; one-shot builds remain an internal pipeline responsibility used by runtime startup and publish |
| `watch` interactive flow | `packages/cli/lib/src/commands/build_command.dart` | Current watch mode is embedded in `build` with stdin commands and hard exit | `packages/cli`, `packages/build_engine`, `packages/runtime_flutter` | remove intentionally | covered | integration tests | local watch moves to `DeckSource.local(watch: true)` and is removed from the public CLI surface |
