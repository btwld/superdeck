...
| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| Canonical deck artifact | `packages/core/lib/src/deck_service.dart`, `docs/reference/contracts.mdx` | `.superdeck/superdeck.json` is the runtime contract | `packages/contracts`, `packages/build_engine` | preserve with versioning | covered | contract tests | rename to `.superdeck/superdeck.v2.json` |
| Full deck debug artifact | `packages/core/lib/src/deck_service.dart`, `packages/core/lib/src/markdown_json.dart` | `.superdeck/superdeck_full.json` expands markdown blocks into AST JSON | `packages/contracts`, `packages/build_engine` | preserve | covered | integration tests | rename to `.superdeck/superdeck_full.v2.json` |
| Generated assets manifest | `packages/core/lib/src/models/asset_model.dart`, `packages/core/lib/src/deck_service.dart` | Timestamp + file list manifest for generated assets | `packages/contracts`, `packages/build_engine` | preserve | covered | contract tests, integration tests | rename to `.superdeck/generated_assets.v2.json` |
| Build status artifact | `packages/core/lib/src/deck_service.dart` | Machine-readable `unknown/building/success/failure` state | `packages/contracts`, `packages/build_engine` | preserve | covered | integration tests | rename to `.superdeck/build_status.v2.json` |
...
| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| File-backed deck loading | `packages/core/lib/src/deck_service.dart`, `packages/superdeck/lib/src/runtime/deck_config.dart`, `packages/superdeck/lib/src/runtime/superdeck_provider.dart` | Debug IO runtimes where `kCanRunProcess` is true load from the filesystem through `SuperDeckProvider` and can stream updates from `DeckService.loadDeckStream()` | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Bundled deck loading | `packages/superdeck/lib/src/runtime/bundled_deck_service.dart`, `packages/superdeck/lib/src/runtime/deck_config.dart`, `packages/superdeck/lib/src/runtime/superdeck_provider.dart` | Web/test/release-like runtimes load bundled JSON assets through `SuperDeckProvider` instead of file-backed streaming | `packages/runtime_flutter` | preserve | covered | runtime tests | bundled runtimes use canonical bundled artifact paths only; custom `outputDir` / `assetsPath` remain a local dev/process-capable concern |
| Route shape | `packages/superdeck/lib/src/runtime/navigation/navigation_service.dart` | Router uses `/slides/:index` with root redirect | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Keyboard navigation | `packages/superdeck/lib/src/runtime/navigation/navigation_events.dart`, `packages/superdeck/lib/src/runtime/navigation/navigation_input_listener.dart` | Meta + arrows navigate slides | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Touch navigation | `packages/superdeck/lib/src/runtime/navigation/navigation_events.dart`, `packages/superdeck/lib/src/runtime/navigation/navigation_input_listener.dart` | Tap left/right and swipe navigate on touch devices | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
...
| Slide parts | `packages/superdeck/lib/src/presentation/slide_frame.dart`, docs | Header/footer/background can be customized and read slide context | `packages/runtime_flutter` | preserve | covered | runtime/widget tests | none |
| Template system | `packages/superdeck/lib/src/presentation/slide_template.dart`, `packages/superdeck/lib/src/presentation/template_resolver.dart` | Templates bundle parts + style systems, with `template: none` opt-out | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Style merge from code + YAML | `packages/superdeck/lib/src/styling/schema/style_config.dart` | Internal-only `StyleConfigLoader` can load `styles.yaml` and merge it with code-defined styles, with code winning conflicts; this is not automatic runtime startup behavior today | `packages/runtime_flutter` | preserve | covered | runtime tests, API audit | external YAML loading stays opt-in and explicitly deferred from the current runtime-local contract; the old `DeckOptions`-based helper is no longer part of the public styling surface |
| Built-in widget registry | `packages/superdeck/lib/src/widgets/widgets.dart`, `packages/superdeck/lib/src/slides/slide_data_builder.dart` | `image`, `dartpad`, `qrcode` are always available and can be overridden | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Custom widget parse/build contract | `packages/superdeck/lib/src/presentation/block_definition.dart`, `packages/superdeck/lib/src/rendering/blocks/block_widget.dart` | `BlockDefinition` parses raw args and builds using block + slide context | `packages/runtime_flutter` | preserve with stronger typing | covered | runtime tests | move to typed outcomes |
| Notes panel | `packages/superdeck/lib/src/ui/app_shell.dart`, `packages/superdeck/lib/src/ui/panels/notes_panel.dart` | Notes panel shows current slide notes using the canonical `notes` runtime field | `packages/runtime_flutter` | preserve | covered | runtime tests | cleanup complete |
...
| Thumbnail generation | `packages/superdeck/lib/src/export/thumbnail_service.dart`, `packages/superdeck/lib/src/runtime/deck_controller.dart` | Runtime thumbnails generated on demand with stale-state issues today | `packages/runtime_flutter` | preserve with cleanup | covered | runtime tests | thumbnails are runtime-owned dev snapshots only, generated in embedded dev/watch mode, consumed as bundled assets or fallback UI elsewhere, and invalidated by `slide key + render signature` |
...
| Plugin routes/actions/floating action | `packages/superdeck/lib/src/presentation/deck_extension.dart`, `packages/superdeck/lib/src/runtime/superdeck_handle.dart`, `packages/superdeck/lib/src/ui/app_shell.dart`, `packages/superdeck/lib/src/ui/panels/bottom_bar.dart` | Extensions contribute routes, inline actions, floating action, and async init | `packages/runtime_flutter` | preserve with stronger lifecycle typing | covered | runtime tests | canonical v2 behavioral surface is `DeckExtension` |
| Rebuild indicator | `packages/superdeck/lib/src/runtime/superdeck_provider.dart`, `packages/superdeck/lib/src/utils/deck_watcher.dart` | Provider-owned lifecycle UI can show rebuild state driven by watcher while keeping the last good deck visible | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Runtime bootstrap | `packages/superdeck/lib/src/runtime/superdeck_init.dart`, `packages/superdeck/lib/src/runtime/superdeck_provider.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart` | Syntax highlighting, extension init, deck loading/watch, and controller wiring happen through `initializeSuperDeck(...)`, `SuperDeckProvider(...)`, and `SuperDeckApp(deck: ...)` | `packages/runtime_flutter` | preserve with cleanup | covered | runtime tests | canonical v2 bootstrap is `initializeSuperDeck(...)` followed by `SuperDeckProvider(config: ..., builder: ...)` and `SuperDeckApp(deck: ...)` |
...
| Export mode rendering | `packages/superdeck/lib/src/slides/slide_data.dart`, `packages/superdeck/lib/src/markdown/markdown_hero_mixin.dart`, `packages/superdeck/lib/src/rendering/blocks/block_widget.dart` | Export mode suppresses interaction-only behavior such as hero flights and scroll behavior | `packages/runtime_flutter` | preserve | covered | runtime tests | none |

## CLI And Operational Surface

| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| `superdeck.yaml` load path | `packages/cli/lib/src/commands/base_command.dart`, `packages/superdeck/lib/src/runtime/deck_config.dart`, `packages/superdeck/lib/src/runtime/superdeck_provider.dart` | CLI reads `superdeck.yaml` strictly; runtime uses explicit `initializeSuperDeck(...)` + `DeckConfig` + `SuperDeckProvider(...)` inputs and does not implicitly resolve local YAML | `packages/build_engine`, `packages/runtime_flutter`, `packages/cli` | change intentionally | covered | integration tests | external config-source behavior is explicitly deferred from the current runtime-local contract and is not a blocker for the current rewrite checklist |
...
| `build` one-shot flow | `packages/cli/lib/src/commands/build_command.dart` | Runs setup checks, build pipeline, and writes artifacts | `packages/cli`, `packages/build_engine`, `packages/runtime_flutter` | preserve with cleanup | covered | integration tests, API audit | `superdeck build` remains a supported transitional public command through v2.0, but the preferred workflow is runtime-first local development plus `publish`; implementation should stay a thin wrapper over the shared v2 pipeline |
| `watch` interactive flow | `packages/cli/lib/src/commands/build_command.dart` | Current watch mode is embedded in `build` with stdin commands and hard exit | `packages/cli`, `packages/build_engine`, `packages/runtime_flutter` | preserve with cleanup | covered | integration tests, API audit | `build --watch` remains transitional public support through v2.0 while `DeckConfig.local(watch: true)` is the preferred workflow; docs and implementation are aligned around that transitional status |
...
| Runtime-triggered watch behavior | `packages/superdeck/lib/src/runtime/deck_config.dart`, `packages/superdeck/lib/src/runtime/superdeck_provider.dart`, `packages/superdeck/lib/src/utils/deck_watcher.dart` | `DeckConfig.local(watch: ...)` drives provider-owned watcher orchestration from the runtime/bootstrap layer | `packages/runtime_flutter`, `packages/cli`, `packages/migration_tools` | preserve with cleanup | covered | runtime + migration tests | automatic rebuild trigger stays limited to `slides.md` while code-side styling/widget changes rely on Flutter hot reload |

## Public API And Migration Surface

| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| Package entry surface | `packages/superdeck/lib/superdeck.dart`, `packages/core/lib/superdeck_core.dart` | Public API currently exposed through barrel exports | `packages/contracts`, `packages/runtime_flutter` | preserve with cleanup | covered | API audit | one canonical primary barrel per package; `package:superdeck/superdeck.dart` must export the canonical v2 runtime/bootstrap API; keep `superdeck_core/asset_cache_store_io.dart` as the only explicit extra helper surface; remove `superdeck_cli/runner.dart` from the supported public API |
...
| `template: none` | `packages/superdeck/lib/src/presentation/template_resolver.dart`, tests | Reserved opt-out of default template behavior | `packages/runtime_flutter`, `packages/contracts` | preserve | covered | runtime tests | none |
...
## Deferred Follow-Up Outside The Current Freeze

1. External config source policy
- `superdeck.yaml` and `styles.yaml` remain outside the current runtime-local contract.
- Revisit them in a dedicated external-config pass rather than treating them as blockers for the current rewrite checklist.
