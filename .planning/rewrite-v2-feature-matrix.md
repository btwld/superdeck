...
| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| File-backed deck loading | `packages/core/lib/src/deck_service.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart`, `packages/superdeck/lib/src/utils/constants.dart` | Debug IO runtimes where `kCanRunProcess` is true load from filesystem and can stream updates through the private runtime bootstrap in `SuperDeckApp` | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Bundled deck loading | `packages/superdeck/lib/src/runtime/bundled_deck_service.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart`, `packages/superdeck/lib/src/utils/constants.dart` | Web/test/release-like runtimes load bundled JSON assets instead of file-backed streaming through the same private runtime bootstrap | `packages/runtime_flutter` | preserve | covered | runtime tests | bundled runtimes use canonical bundled v2 artifact paths only; custom `outputDir` / `assetsPath` remain a local dev/process-capable concern |
...
| Rebuild indicator | `packages/superdeck/lib/src/ui/superdeck_app.dart`, `packages/superdeck/lib/src/ui/app_shell.dart`, `packages/superdeck/lib/src/utils/deck_watcher.dart` | Runtime UI can show rebuild state driven by watcher | `packages/runtime_flutter` | preserve | covered | runtime tests | none |
| Runtime bootstrap | `packages/superdeck/lib/src/utils/app_initialization.dart`, `packages/superdeck/lib/src/runtime/superdeck_runtime.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart` | Syntax highlighting, desktop window init, extension init, controller wiring, and deck watch/bootstrap happen behind `SuperDeckRuntime.create(...)` and `SuperDeckApp(runtime: ...)` | `packages/runtime_flutter` | preserve with cleanup | covered | runtime tests | canonical v2 bootstrap is `SuperDeckRuntime.create(...)` followed by `SuperDeckApp(runtime: ...)` |
...
| Feature | Current implementation refs | Current behavior | V2 owner | Decision | Plan status | Validation gate | Migration note |
|---|---|---|---|---|---|---|---|
| `superdeck.yaml` load path | `packages/cli/lib/src/commands/base_command.dart`, `packages/superdeck/lib/src/runtime/deck_runtime_config.dart`, `packages/superdeck/lib/src/runtime/superdeck_runtime.dart` | CLI reads `superdeck.yaml` strictly; runtime requires explicit `SuperDeckRuntime.create(...)` inputs and does not implicitly resolve local YAML | `packages/build_engine`, `packages/runtime_flutter`, `packages/cli` | change intentionally | covered | integration tests | external config-source behavior is explicitly deferred from the current runtime-local contract and is not a blocker for the current rewrite checklist |
...
| Runtime-triggered watch behavior | `packages/superdeck/lib/src/runtime/deck_source.dart`, `packages/superdeck/lib/src/runtime/superdeck_runtime.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart` | `DeckSource.local(watch: ...)` drives runtime watcher orchestration from the runtime/bootstrap layer | `packages/runtime_flutter`, `packages/cli`, `packages/migration_tools` | preserve with cleanup | covered | runtime + migration tests | automatic rebuild trigger stays limited to `slides.md` while code-side styling/widget changes rely on Flutter hot reload |
