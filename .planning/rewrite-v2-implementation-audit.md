...
|---|---|---|---|---|
| `DeckOptions` internal scaffolding | `packages/superdeck/lib/src/deck/deck_options.dart`, `packages/superdeck/lib/src/runtime/superdeck_runtime.dart`, `packages/superdeck/lib/src/presentation/deck_presentation.dart` | keep as internal compatibility | Still needed as the adapter layer behind `DeckPresentation` and current controller/template internals. | internal simplification slice |
...
| `DeckController` internal runtime state | `packages/superdeck/lib/src/deck/deck_controller.dart`, `packages/superdeck/lib/src/runtime/superdeck_handle.dart`, `packages/superdeck/lib/src/ui/superdeck_app.dart` | keep as internal compatibility | Controller internals still back the runtime and handle, but no longer leak through public-facing widgets. | internal simplification slice |
