...
- one `RuntimeAssetStore` abstraction backs bundled assets, generated assets, and thumbnail cache lifecycles
- IO backends preserve the current “prefer fresher bundled asset over cache entry” behavior
- web backends preserve the current `rootBundle` + in-memory/data-URI behavior for generated bytes
...
- replace `DeckOptions.watchForChanges` usage with explicit watch/dev tooling guidance
