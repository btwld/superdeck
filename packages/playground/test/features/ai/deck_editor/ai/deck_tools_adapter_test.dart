...
void main() {
  test('registers exactly the seven deck-edit tools with provider schemas', () {
  ...
  test(
    'readSlide and updateStyle reach their queued service operations',
    () async {
      ...
      final readResult = await adapter.tools
          ...
          .call({'index': 0});
      final styleResult = await adapter.tools
          .singleWhere((tool) => tool.name == 'updateStyle')
          .call({
            'style': {
              'name': 'Test',
              'colors': {
      ...
      expect((styleResult['style'] as Map)['name'], 'Test');
    },
  );
}
