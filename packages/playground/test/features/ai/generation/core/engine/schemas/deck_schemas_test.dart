...
void main() {
  ...
  group('buildAiSlideSchema() parity with the canonical contract', () {
    test(
      ...
      () {
        ...
        expect(buildAiSlideSchema().safeParse(slide).isOk, isTrue);
    ...
    test('rejects non-positive flex like core', () {
      for (final flex in [0, -1]) {
        expect(
          buildAiSlideSchema()
              .safeParse(validSlide(block: {'flex': flex}))
              .isOk,
          isFalse,
        ...
        expect(
          buildAiSlideSchema()
              .safeParse(validSlide(section: {'flex': flex}))
              .isOk,
          isFalse,
    ...
    test('spacing is section-only like core', () {
      expect(
        buildAiSlideSchema()
            .safeParse(validSlide(section: {'spacing': 40}))
            .isOk,
        isTrue,
      );
      expect(
        buildAiSlideSchema().safeParse(validSlide(block: {'spacing': 40})).isOk,
        isFalse,
      );
    });
    ...
    test('block surface matches the core reserved widget fields', () {
      final blockProperties =
          ((((buildAiSlideSchema().toJsonSchema()['properties']
                              as Map)['sections']
                          as Map)['items']
                      as Map)['properties']
                  as Map)['blocks']
