...
import 'package:playground/features/ai/generation/core/engine/services/deck_plan_validator.dart';
import 'package:playground/features/ai/generation/core/engine/services/google_schema_adapter.dart';
...
void main() {
  ...
  test('rejects duplicate or empty slide keys after schema parsing', () {
    ...
    expect(
      validateDeckPlanIssues(plan).messages,
      containsAll([
  ...
  test('rejects a plan that does not match the requested slide count', () {
    ...
    expect(
      validateDeckPlanIssues(plan, expectedSlideCount: 3).messages,
  ...
  test('validates deterministic fifteen- and twenty-slide blueprints', () {
    for (final count in [15, 20]) {
      ...
      expect(
        validateDeckPlanIssues(plan, expectedSlideCount: count).messages,
        isEmpty,
  ...
  test('allows a treatment run when compositions still vary', () {
    ...
    final errors = validateDeckPlanIssues(DeckPlan.parse(data)).messages;
  ...
  test('rejects a stale theme and broken section membership', () {
    ...
    final errors = validateDeckPlanIssues(plan).messages;
  ...
  test('rejects changes to exact requested theme and font selections', () {
    ...
    final errors = validateDeckPlanIssues(
      plan,
  ...
  test('requires an exact style and enforces the generated-image budget', () {
    ...
    final withoutStyle = validateDeckPlanIssues(
      plan,
      request: const DeckGenerationRequest(
        userIntent: 'Explain reliable delivery.',
        slideCount: 10,
    ...
    final overBudget = validateDeckPlanIssues(
      plan,
      request: const DeckGenerationRequest(
        userIntent: 'Explain reliable delivery.',
        slideCount: 10,
  ...
  test('rejects an invented visible domain but allows a supplied one', () {
    ...
    final rejected = validateDeckPlanIssues(
      plan,
      request: const DeckGenerationRequest(
    ...
    final allowed = validateDeckPlanIssues(
      plan,
      request: const DeckGenerationRequest(
  ...
  test('requires unsupported numeric claims to be visibly qualified', () {
    ...
    final plannedTarget = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent:
    ...
    final normalizedNumberWords = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final structuralCounts = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final inventedZero = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'Describe onboarding with no change to source systems.',
        slideCount: 10,
    ...
    final qualitativeZero = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final calendarQuarter = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final fractionalQuarter = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'Describe a roadmap direction.',
        slideCount: 10,
    ...
    final structuralSourceDoesNotGroundDuration = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final qualifiedDuration = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'Bring the evidence into one workspace.',
        slideCount: 10,
    ...
    final structuralSourceDoesNotGroundMetric = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
  ...
  test('rejects an unsupported commercial commitment', () {
    ...
    expect(
      ...
      ).messages.singleWhere((error) => error.contains('soc2')),
  ...
  test('rejects invented availability, production, and delivery claims', () {
    ...
    final negated = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final explicitlyHypothetical = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
  ...
  test('requires beta capabilities and commercial shapes to stay qualified', () {
    ...
    final qualified = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent:
            'Cover the evidence inbox, API extensibility, onboarding, pricing '
  ...
  test('rejects exact metrics inflated with an unsupplied plus sign', () {
    ...
    final exactOnly = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final explicitlyOpenEnded = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'The beta included 38+ design partners.',
        slideCount: 10,
  ...
  test('reports a supplied metric reused with a different meaning', () {
    ...
    final changedMeaning = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
    ...
    final preservedMeaning = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'Teams spent 42% less weekly synthesis time.',
        slideCount: 10,
    ...
    final standaloneMetric = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent: 'Teams spent 42% less weekly synthesis time.',
        slideCount: 10,
    ...
    final normalizedWording = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent:
    ...
    final changedCohortMeaning = validateDeckPlanIssues(
      DeckPlan.parse(data),
      request: const DeckGenerationRequest(
        userIntent:
