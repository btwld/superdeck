...
class ThumbnailPreviewService {
  ...
  Future<List<(int, Uint8List)>> generatePreviews({
    ...
    final slideConfigs = await buildRuntimeSlideConfigurations(
      slides: slides,
      options: options,
    );
    ...
    for (var i = 0; i < slideConfigs.length; i++) {
      ...
      try {
        ...
        if ((isCancelled?.call() ?? false) || !context.mounted) {
          debugLog.log('THUMBNAIL', 'Generation stopped after slide $i');
          break;
        }
