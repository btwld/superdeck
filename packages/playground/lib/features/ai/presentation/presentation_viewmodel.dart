...
class PresentationViewModel implements Disposable {
  ...
  Future<void> generate({
    ...
    if (_disposed) return;
    if (_status.value == GenerationStatus.generating) return;
    ...
    try {
      final result = await callback(
        context,
        (phase, imageProgress) =>
            _onProgress(phase, imageProgress, generationEpoch: generationEpoch),
        isCancelled: () => !_isCurrentGeneration(generationEpoch),
      );
      if (!_isCurrentGeneration(generationEpoch)) return;
  ...
  bool _isCurrentGeneration(int generationEpoch) =>
      !_disposed && generationEpoch == _generationEpoch;
  ...
  void addThumbnailPreview(
    ...
    if (_disposed || epoch != _thumbnailEpoch) return;
