...
@visibleForTesting
Set<String> referencedGeneratedAssetFilenames(
  List<Map<String, dynamic>> slides,
) {
  final filenames = <String>{};
  for (final slide in slides) {
    _collectReferencedGeneratedAssets(slide, filenames);
  }
  return filenames;
}

void _collectReferencedGeneratedAssets(Object? value, Set<String> filenames) {
  if (value is String) {
    for (final match in RegExp(
      r'(?:\.superdeck/assets/|assets/)([A-Za-z0-9._-]+\.png)',
    ).allMatches(value)) {
      final filename = match.group(1);
      if (filename != null &&
          (filename.startsWith('slide-') ||
  ...
  if (value is Map) {
    for (final child in value.values) {
      _collectReferencedGeneratedAssets(child, filenames);
    }
    return;
  }
