...
/// stays the same, so a plain pass would reuse the stale capture. The options
/// effect therefore force-regenerates every thumbnail — active slide
/// included, so it is correct the instant the caret leaves it.
class ThumbnailRefresher extends StatefulWidget {
  const ThumbnailRefresher({required this.child, this.regenerate, super.key});
...
class _ThumbnailRefresherState extends State<ThumbnailRefresher> {
  ...
  /// context is laid out and overlapping triggers do not stack. A pending
  /// [force] request wins over a coalesced plain one.
  void _scheduleGenerate({bool force = false}) {
