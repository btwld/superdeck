...
/// A [DocumentRange] spanning `[start, end)` within a single node.
DocumentRange _nodeRange(String nodeId, int start, int end) => DocumentRange(
...
abstract class TextNodeAttributionReaction extends EditReaction {
  ...
  void modifyContent(
    ...
    for (final node in document) {
      ...
      final nodeRange = _nodeRange(node.id, 0, text.length);
...
class BlockHighlightReaction extends EditReaction {
  ...
  void modifyContent(
    ...
    for (final node in document) {
      ...
      requestDispatcher.execute([
        RemoveTextAttributionsRequest(
          documentRange: _nodeRange(node.id, 0, text.length),
          attributions: attributions,
        ),
      ]);
  ...
  void _addAttribution(
    ...
    requestDispatcher.execute([
      AddTextAttributionsRequest(
        documentRange: _nodeRange(nodeId, start, end),
