  ...
  runApp(
    SuperDeckProvider(
      ...
      builder: (context, deck) => SuperDeckApp(
        deck: deck,
        theme: const DeckTheme(debug: true),
      ),
    ),
  );
}
...
Use `DeckConfig.local(watch: true)` during development. SuperDeck will:
...
- Use `DeckConfig.bundle(...)` instead of `DeckConfig.local(...)` for bundled/web-only deployments

**Changes not appearing**
- Confirm `DeckConfig.local(watch: true)` is enabled
