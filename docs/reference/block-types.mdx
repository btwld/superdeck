...
Widget name from `DeckTheme.widgets`.
...
class CustomChartBlockDefinition extends BlockDefinition<Map<String, Object?>> {
  const CustomChartBlockDefinition();
...
runApp(
  SuperDeckProvider(
    ...
    builder: (context, deck) => SuperDeckApp(
      ...
      theme: DeckTheme(
        widgets: const {
          'customChart': CustomChartBlockDefinition(),
        },
      ),
    ),
  ),
...
**Note:** Use explicit (`@widget { name: "chart" }`) or shorthand (`@chart`) syntax. All properties pass to `BlockDefinition.parse()` as `Map<String, Object?>`.
