...
| `padding` | `object?` | Normalized physical edges (`top`, `right`, `bottom`, `left`) |
...
`name`, `align`, `flex`, `padding`, `scrollable`, and `type` are reserved block
fields and are not included in `WidgetBlock.args`. Built-in widget fields such
as image `scale` remain widget arguments and are validated by the built-in DTO.

## Padding normalization

Authoring accepts a scalar, a symmetric object, or a physical-edge object:
...
Compiled contracts always emit four physical edges. Symmetric and edge keys
cannot be mixed; empty/unknown-key objects and negative or non-finite values are
invalid.

## Contract evolution and downgrade

`spacing` and `padding` are optional/defaulted, so the next-major runtime reads
existing 1.x contracts. A 1.x runtime is not required to read contracts authored
