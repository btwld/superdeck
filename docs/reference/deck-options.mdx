...
A Markdown block-level `padding` value is applied after these Mix variants
resolve. It replaces only the resolved `BoxSpec.padding`; margin, border,
decoration, animation, widget modifiers, and the active selector remain intact.
