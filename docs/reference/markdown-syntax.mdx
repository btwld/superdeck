...
In the standard `superdeck build` and watch flows, fenced `dart` blocks are reformatted during build before the slide is rendered.
...
The current parser also accepts a legacy compatibility form without braces after the language token, but the braced form is the canonical syntax.
