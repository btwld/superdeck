...
SuperDeck calls `BlockDefinition.parse()` with the block's arguments and then `BlockDefinition.build()` to render your widget. For complex widgets, parse into a typed args class and validate with `Ack` (see the Custom Widgets guide).
...
Import `package:flutter/material.dart` and `package:mix/mix.dart` to access the builders above. Use `customStyle` as `DeckTheme.baseStyle` or as a named style variant.
...
SuperDeck uses a two-stage pipeline:

1. **Local runtime build**: `DeckConfig.local(...)` parses `slides.md`, renders Mermaid diagrams to images, and generates asset manifests
2. **Runtime rendering**: Flutter renders the parsed slide data as widgets with full styling control
