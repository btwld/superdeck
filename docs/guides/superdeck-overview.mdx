...
Import `package:flutter/material.dart` and `package:mix/mix.dart` to access the builders above. Use `customStyle` as `DeckPresentation.baseStyle` or as a named style variant.
...
1. **Local runtime build**: `DeckSource.local(...)` parses `slides.md`, renders Mermaid diagrams to images, and generates asset manifests
