...
Use `@block` as the canonical markdown-content directive.
...
Use HTML comment delimiters for speaker notes:
...
In the standard local runtime build flow, fenced `dart` blocks are reformatted before they are rendered on a slide. If you pass fenced-code options, prefer the braced header form; the parser also accepts an unbraced form without options after the language token.
...
SuperDeck renders diagrams to images during the local runtime build and publish flows. During development, `DeckConfig.local(watch: true)` rebuilds them when `slides.md` changes.
