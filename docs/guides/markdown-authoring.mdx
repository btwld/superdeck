...
> `@column` is not supported in 1.0. Replace each legacy `@column` body with `@block`. Keep `@section` as the horizontal container for multi-column layouts.
...
The CLI renders diagrams to images during build. Run `dart run superdeck_cli:main build` after editing diagrams.
