...
Built-in widgets (`image`, `dartpad`, `qrcode`) use the same syntax. Mermaid diagrams use fenced code blocks (` ```mermaid`).
...
Properties: `src` (required; asset path, absolute file path, or URL), `fit` (`contain`, `cover`, `fill`, `fitWidth`, `fitHeight`, `none`, `scaleDown`), `width`, `height`
