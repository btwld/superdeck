...
In the standard `superdeck build` and watch flows, fenced `dart` blocks are reformatted during build before they are rendered on a slide. If you pass fenced-code options, prefer the braced header form; the current parser also accepts a legacy compatibility form without braces after the language token.
