...
SuperDeck renders Mermaid diagrams to PNG during the local runtime build and publish flows. By default, the builder uses Mermaid’s `base` theme with a dark set of `themeVariables`.
...
When SuperDeck runs a local or publish build, Mermaid diagrams are automatically:
