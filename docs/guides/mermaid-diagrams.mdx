...
`MermaidGenerator` uses Mermaid’s default theme with dark-mode color derivation and a transparent background by default.
...
It scans content blocks for fenced Mermaid code, renders each diagram to a PNG,
writes the generated asset under the active SuperDeck build output directory,
and replaces the fence with Markdown image syntax that points at the generated
asset.
