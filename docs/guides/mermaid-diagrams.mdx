...
SuperDeck renders Mermaid diagrams to PNG during `dart run superdeck_cli:main build`. By default, the builder uses Mermaid’s `base` theme with a dark set of `themeVariables`.
...
When you run `dart run superdeck_cli:main build`, Mermaid diagrams are automatically:
