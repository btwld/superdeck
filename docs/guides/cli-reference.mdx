...
Local slide build/watch is owned by the runtime through `DeckSource.local(...)`,
...
v2.0 command. For day-to-day app development, prefer runtime watch with
`DeckSource.local(watch: true)` and `flutter run`.
...
│   ├── superdeck_full.v2.json  # Includes markdown AST JSON (for debugging)
│   ├── generated_assets.v2.json # Asset manifest
│   ├── build_status.v2.json    # Last build status
...
Use runtime-owned local build/watch:
...
**Solution:** Ensure you're editing `slides.md` in the project root and that your app uses `DeckSource.local(watch: true)`
...
1. **Use runtime watch** - `DeckSource.local(watch: true)` is the primary development flow
