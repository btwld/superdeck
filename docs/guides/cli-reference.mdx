...
`--base-href` from your `origin` remote, and publishes the output to a branch
(`gh-pages` by default) using an isolated git worktree — your working tree is
never touched.
...
Each publish applies the GitHub Pages conventions a Flutter web app needs:
`.nojekyll` (so `assets/` and `canvaskit/` are served as-is), a `404.html` SPA
fallback for deep links, and a preserved `CNAME` for custom domains (set one
with `--cname deck.example.com`). Publishes extend the branch's existing history
rather than orphaning it, so prior deploys aren't clobbered.
...
For Firebase, use the official tooling rather than `superdeck-deploy`. In CI, the
...
with `firebase init hosting:github`. Locally, run `firebase deploy --only hosting`.
