...
The SuperDeck CLI provides tools for creating, building, and publishing presentations. This reference covers all commands and options.
...
Create a SuperDeck starter app or refresh starter files in an existing app. Existing-app refreshes update the starter deck content, `pubspec.yaml`, and the web loader without replacing your app code or platform folders.
...
| Option | Short | Description |
|--------|-------|-------------|
| `--force` | `-f` | Skip the refresh confirmation prompt |
...
│       └── mermaid_<hash>.png
└── web/
    └── index.html              # Starter web shell with loader
```
...
**Solution:** Ensure `slides.md` exists in project root. If this app was created with SuperDeck, run `superdeck create .` to restore the starter deck files and web loader.
