...
│       └── mermaid_<hash>.png
└── web/
    ├── index.html              # Flutter web shell patched with loader markup
    ├── flutter_bootstrap.js    # SuperDeck-managed loader lifecycle
    └── superdeck_loader.svg    # Loader artwork
```
...
`No changes to commit` is not a failure — it means the built output matches
what is already published. The command exits successfully. If you expected a
republish, make a change (or run `superdeck build --force-rebuild`) and run
`superdeck publish` again.
