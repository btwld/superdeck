...
Build presentations from markdown files. Parses `slides.md`, generates presentation data in `.superdeck/superdeck.json`, and creates optimized build-time assets (for example Mermaid PNGs).
...
│       ├── mermaid_<hash>.png
│   └── thumbnails/             # Runtime thumbnail path (when writable)
│       └── thumbnail_<slideKey>.png
└── web/
    └── index.html              # Custom index with loading indicator
```

`superdeck build` does not pre-generate slide thumbnails in the current pipeline.
Thumbnails are generated on-demand at runtime when missing, then cached.
On IO platforms, SuperDeck can fall back to the app cache directory if `.superdeck/thumbnails` is not writable.
On web, thumbnail bytes are persisted in browser storage.
