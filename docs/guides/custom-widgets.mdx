...
1. **Define widget builders** in your Flutter app's `PresentationOptions`
...
### 1. Define Widget Builders in PresentationOptions
...
```
- Ensure the widget name is registered in `PresentationOptions.widgets`
...
1. **Enable debug mode** - Use `debug: true` in `PresentationOptions`
...
- Check [PresentationOptions Reference](/docs/reference/presentation-options) for complete API
