...
`DeckTheme.widgets`, then use them in `slides.md`.
...
All properties pass to `BlockDefinition.parse()` as arguments.
...
class TwitterDefinition extends BlockDefinition<TwitterArgs> {
...
In `BlockDefinition.build()`:

- `BlockContext.of(context)` - Block size, alignment, slide spec
- `SlideData.of(context)` - Slide index, options, metadata
...
- **"Widget not found"** - Verify the name is registered in `DeckTheme.widgets`
